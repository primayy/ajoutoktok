...
class chatRoom(QWidget):
    ...
    def initUI(self):
        ...
        commend = "chat_client " + str(self.parent.course[1]) + " " + str(
            self.tab.tabText(self.tab.currentIndex()))  # 과목코드 + 카테고리명
    ...
    def getChatHistory(self):
        ...
        commend = 'chat_history ' + self.lecId + " " + self.tab.tabText(
            self.tab.currentIndex()) + " " + self.parent.stuid
    ...
    def sendMsg(self, msg):
        ...
        if msg != "":
            if self.sendType == False:
                ...
                commend = 'sendMsg ' + self.tab.tabText(
                    self.tab.currentIndex()) + " " + self.lecId + " " + msg + " " + self.parent.stuid
                ...
                chat_id = tmp.split(' ')
                # chat server에 전송 -> 모두에게 뿌리기 위해
                data = 'chat_update ' + chat_id[1] + " " + self.parent.stuid + " " + self.parent.course[
                    1] + " " + self.tab.tabText(self.tab.currentIndex())
...
class sendQuestion(QWidget):
    ...
    def sendToEmail(self):
        ...
        commend = "sendToEmail " + self.parent.tab.tabText(
            self.parent.tab.currentIndex()) + " " + self.parent.lecId + " " + self.email.text()
