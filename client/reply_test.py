...
class replyRoom(QWidget):
    def __init__(self,parent,grandparent):
        ...
        self.lecId = self.parent.getLecId()
        ...
        if len(self.history) != 0:
            self.history.pop()
        self.parent.tab.currentChanged.connect(self.parent.category_changed)
        ...
        #채팅 내용
        self.replyWidget = QWidget()
        self.replyWidget.setStyleSheet('background-color:white')
        self.replyLayout = QVBoxLayout()
        self.replyWidget.setLayout(self.replyLayout)

        #입력창
        self.reply_input = QTextEdit()
        self.reply_input.setStyleSheet('background:white')

        self.replyLayoutWidget = QWidget()
        self.replyLayoutWidget.setStyleSheet('background-color:#5218FA')
        self.replyLayoutWidget.setMaximumHeight(100)
        self.replyInputLayout = QHBoxLayout()
        self.replyLayoutWidget.setLayout(self.replyInputLayout)

        #메인 레이아웃 설정
        self.mainLayout.setContentsMargins(0,0,0,0)
        self.mainLayout.addWidget(self.titleWidget)
        self.mainLayout.addWidget(self.replyWidget)
        self.mainLayout.addWidget(self.replyLayoutWidget)
    ...
    def initUI(self):
        # 쓰레드
        self.t = update_listener(self.parent)
        self.t.chatUpdate.connect(self.parent.chat_Update)
        self.t.start()
...
class chatWidget(QWidget):
    def __init__(self,grandparent,comments,Ggrandparent):
        super().__init__()
        self.parent = parent
        self.grandparent = grandparent
        ...
        self.clientSocket = grandparent.clientSocket
    ...
    def likeClicked(self):
        print(self.comments)
        commend = 'like_update '+ self.parent.comments[6] + " " + self.grandparent.stuid#학번 + msg
        self.parent.clientSocket.send(commend.encode('utf-8'))
        print(commend)
        result = self.parent.clientSocket.recv(1024)
        result = result.decode('utf-8')
        print(result)
        self.grandparent.category_changed()
